        <div class="user_right">
			...
			<div class="user_right_content">
				...
				<div class="user_list">
					...
					<dl>
						<dd class="width_list_1" style="border-left:#eeeeee 1px solid;text-align:left;padding-left:10px;">
						<?php
						if($row->status ==2)
						{
							echo '<a href="javascript:void(0);">'.$row->title.'</a><span class="del">已删除</span>';
						}
						else
						{
							echo '<a href="'.site_url('/data/show').'/'.$row->data_uuid.'">'.$row->title.'</a>';
						}
						?></dd>
						<dd class="width_list_2"><?php echo $row->name;?></dd>
						<dd class="width_list_3"><?php echo date('Y-m-d H:i:s',$row->create_time);?></dd>
						<dd class="width_list_2"><a href="javascript:void(0);" onclick="del('<?php echo $row->data_collection_uuid;?>')">取消收藏</a></dd>
					</dl>
					<?php endforeach;?>
				</div>
...
{
	$.get("<?php echo site_url('user/favorites/del/') . '/' ?>" + collection_uuid ,function(data){
		location.reload();
	});
}
</script>